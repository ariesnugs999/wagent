...
class Products extends MY_Controller
{
    ...
    public function getadjustments($warehouse_id = null)
    {
        ...
        $delete_link = "<a href='#' class='tip po' title='<b>" . $this->lang->line('delete_adjustment') . "</b>' data-content=\"<p>"
            ...
            . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fas fa-trash-alt\"></i></a>";
    ...
    public function getProducts($warehouse_id = null)
    {
        ...
        $detail_link = anchor('admin/products/view/$1', '<i class="far fa-file-alt"></i> ' . lang('product_details'));
    ...
    public function index($warehouse_id = null)
    {
        ...
        } else if ($this->Agen || !$this->session->userdata('warehouse_id')) { 
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'sync_quantity') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('X1', lang('details'));
                    $this->excel->getActiveSheet()->SetCellValue('Y1', lang('product_details'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('X' . $row, $product->details);
                        $this->excel->getActiveSheet()->SetCellValue('Y' . $row, $product->product_details);
                        $row++;
                    }
